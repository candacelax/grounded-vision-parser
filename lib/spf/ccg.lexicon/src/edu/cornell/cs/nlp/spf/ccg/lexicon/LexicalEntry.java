...
public class LexicalEntry<MR> implements Serializable {
	...
	/**
	 * determines if the semantics are empty, e.g. (lambda $0:<e,t> (lambda $1:e ($0 $1)))
	 */
	public boolean emptySemantics(){
		String[] semantics = category.getSemantics().toString().split(" ");
		
		for (String s : semantics){
			String t = s.replaceAll("[(]|[)]", "");
			
			if (!t.equals("lambda") && !t.equals("and:<t*,t>") && !t.startsWith("$")){
				return false;
			}
		}
		return true;
