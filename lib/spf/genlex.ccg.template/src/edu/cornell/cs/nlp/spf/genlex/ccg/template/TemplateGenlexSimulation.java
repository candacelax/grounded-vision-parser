 ...
 */
public class TemplateGenlexSimulation<SAMPLE extends Sentence, DI extends ILabeledDataItem<SAMPLE, LogicalExpression>> extends
	...
	public static final ILogger							LOG					= LoggerFactory
			.create(TemplateGenlexSimulation.class);
	...
	protected TemplateGenlexSimulation(
	...
	public ILexicon<LogicalExpression> generate(DI dataItem,
		...
		for (final IDerivation<LogicalExpression> parse : parserOutput
				.getAllDerivations()) {
			for (final LexicalEntry<LogicalExpression> entry : parse.getMaxLexicalEntries()) {
		...
		for (final Triplet<LexicalTemplate, TokenSeq, List<String>> triplet : triplets) {
			...
			for (final List<LogicalConstant> seq : fineRepository.getConstantSeqs(template)) {
				final Lexeme lexeme = new Lexeme(tokens, seq, attributes, entryProperties);
	...
	public static class Builder<SAMPLE extends Sentence, DI extends ILabeledDataItem<SAMPLE, LogicalExpression>> {
		...
		public TemplateGenlexSimulation<SAMPLE, DI> build() {
			final GenerationRepository repository = new GenerationRepository();
			
			return new TemplateGenlexSimulation<SAMPLE, DI>(
	...
	public static class Creator<SAMPLE extends Sentence, DI extends ILabeledDataItem<SAMPLE, LogicalExpression>>
			implements IResourceObjectCreator<TemplateGenlexSimulation<SAMPLE, DI>> {

		private final String type;

		public Creator() {
			this("genlex.template.simulation");
		}
		...
		@Override
		public TemplateGenlexSimulation<SAMPLE, DI> create(Parameters params,
		...
		public ResourceUsage usage() {
			return ResourceUsage.builder(type, TemplateGenlexSimulation.class)
