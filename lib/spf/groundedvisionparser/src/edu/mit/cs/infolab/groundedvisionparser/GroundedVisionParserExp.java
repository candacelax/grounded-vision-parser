...
public class GroundedVisionParserExp extends DistributedExperiment {
	public static final ILogger						LOG	= LoggerFactory
																.create(GroundedVisionParserExp.class);
	public static final InputStream inputStream = System.in;
	
	private final LogicalExpressionCategoryServices	categoryServices;

	public GroundedVisionParserExp(File initFile) throws IOException, ClassNotFoundException {
		this(initFile, Collections.<String, String> emptyMap(),
				new GroundedVisionParserResourceRepo());
	}

	public GroundedVisionParserExp(File initFile, Map<String, String> envParams,
			ResourceCreatorRepository creatorRepo) throws IOException, ClassNotFoundException {
		...
		try {
			...
			LogicLanguageServices
					.setInstance(new LogicLanguageServices.Builder(
							...
							//new StrictTypeComparator())
							.setNumeralTypeName("i")
							.setUseOntology(true)
							//.setComparator(new StructureOnlyComaprator(new LogicalExpressionComparator())) // Set the comparisons only to structure 
		...
		// //////////////////////////////////////////////////
		// To turn on please add the types into ontology file
		// unfactored ontology files
		FactoringServices.set(new FactoringServices.Builder()
				//.addConstant(LogicalConstant.read("exists:<<e,t>,t>"))
				//.addConstant(LogicalConstant.read("the:<<e,t>,e>"))
				.build());
		//		
		// FactoringServices.set(new FactoringServices.Builder()
	...
	private Job createJob(Parameters params) throws ClassNotFoundException, IOException {
	...
	private Job createModelInitJob(Parameters params) throws FileNotFoundException {
	...
	private Job createTestJob(Parameters params) throws FileNotFoundException {
		...
		// Make the stats
		final ExactMatchTestingStatistics<Sentence, LogicalExpression, SingleSentence> stats = 
				new ExactMatchTestingStatistics<Sentence, LogicalExpression, SingleSentence>();
	...
	@SuppressWarnings("unchecked")
	private Job createTrainJob(Parameters params) throws ClassNotFoundException, IOException {
	
		// The model to use
		final Model<Sentence, LogicalExpression> model = (Model<Sentence, LogicalExpression>) get(params.get("model"));
		
		// The learning
		final ILearnerGrounded<Sentence, SingleSentence, Model<Sentence, LogicalExpression>> learner = 
				(ILearnerGrounded<Sentence, SingleSentence, Model<Sentence, LogicalExpression>>) get(params
				.get("learner"));
