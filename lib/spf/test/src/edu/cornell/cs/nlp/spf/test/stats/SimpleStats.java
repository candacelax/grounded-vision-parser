...
public class SimpleStats<SAMPLE> implements IStatistics<SAMPLE> {
	...
	/**
	 * Number of correct with partial match
	 */
	private int					partialCorrects		= 0;

	/**
	 * Number of inference failures with partial match
	 */
	private int					partialFailures		= 0;

	/**
	 * Number of incorrects with partial match
	 */
	private int					partialIncorrects	= 0;
	...
	/**
	 * Total number of inference attempts for partial match.
	 */
	...
	@Override
	public double getPartialCorrects() {
		return partialCorrects;
	}

	@Override
	public double getPartialFailures() {
		return partialFailures;
	}

	@Override
	public double getPartialIncorrects() {
		return partialIncorrects;
	}
	
	@Override
	public void recordCorrectPartial(SAMPLE dataItem) {
		LOG.info("[%s stats]  Record partial correct.", getLabel());
		partialTotal++;
		partialCorrects++;
	}

	@Override
	public void recordFailurePartial(SAMPLE dataItem) {
		LOG.info("[%s stats]  Record partial failure.", getLabel());
		partialTotal++;
		partialFailures++;
	}

	@Override
	public void recordIncorrectPartial(SAMPLE dataItem) {
		LOG.info("[%s stats]  Record partial incorrect.", getLabel());
		partialTotal++;
		partialIncorrects++;
	}
	...
	@Override
	public double partialF1() {
		return partialPrecision() + partialRecall() == 0.0 ? 0.0 : 2
				* partialPrecision() * partialRecall()
				/ (partialPrecision() + partialRecall());
	}

	@Override
	public double partialPrecision() {
		return partialTotal - partialFailures == 0.0 ? 0.0
				: (double) partialCorrects
						/ (partialTotal - partialFailures);
	}

	@Override
	public double partialRecall() {
		return partialTotal == 0.0 ? 0.0 : (double) partialCorrects
				/ partialTotal;
	}
