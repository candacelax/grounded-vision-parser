...
public interface ITestingStatistics<SAMPLE, LABEL, DI extends ILabeledDataItem<SAMPLE, LABEL>> {
	...
	void recordNoParsePartialMatch(DI dataItem);
	 ...
	 * Record a parse.
	 */
	// changed by Candace; added isCorrect()
	void recordParse(DI dataItem, LABEL candidate, boolean isCorrect);
	
	void recordParsePartialMatch(DI dataItem, LABEL candidate, boolean isCorrectPartialMatch);

	void recordParses(DI dataItem, List<LABEL> candidates);
	
	void recordParsesPartialMatch(DI dataItem, List<LABEL> candidates);
	 ...
	 * -- added by Candace -- 
	 */
	void writeAveragesToFile(String filename, Integer size_test_set);
	void writeFinalAccuracy(String filename);
}
